...
# Calculate latency for given actions
    def calculate_latency(self, obs, actions):
        latency = obs[0]
        lamda = obs[1]
        miuAccesNet = obs[2]
        miuCoreNet = obs[3]
        miuCloudNet = obs[4]
        Ac=actions
        action = self.Load_Traffic(Ac)
        
        for i in range(len(latency)):
            for j in range(len(latency[i])):
                # AccesNetLamda = CEF_SiAdelayCore4T60k.get_AccesNetwork_lamda(i,j, trafic)
                # # print('AccesNetLamdadddddddddd', AccesNetLamda)
                
                for k in range(len(latency[i][j])):
                
                    AccesNetLamda = CEF_SiAdelayCore4T60k.get_AccesNetwork_lamda(i,j, action)
                    # print('AccesNetLamdadddddddddd', AccesNetLamda)
                    CoreNetLamda = CEF_SiAdelayCore4T60k.get_CoreNetwork_lamda(j, action)
                    # print('lamdaCoreNet', CoreNetLamda)
                    CloudNetLamda = CEF_SiAdelayCore4T60k.get_CloudNetwork_lamda(action)
                    # print('lamdaAccesNet', CloudNetLamda)
                    
                    # AccesLamda = CEF_SiAdelayCore4T60k.get_AccesNetwork_lamda(i,j, trafic)
                    ...
                    #     # print("AccesNetLamda", AccesNetLamda)
                    # else:
                    #     AccesNetLamda = CEF_SiAdelayCore4T60k.get_AccesNetwork_lamda(i,j, trafic) * 0.80
                    #     # print("Else_AccesNetLamda", AccesNetLamda)
                    # CoreLamda = CEF_SiAdelayCore4T60k.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.40)
                    # # print("CoreNetLamda", CoreNetLamda)
                    # if (self.miuCoreNetwork[i] <= CoreLamda):
                    #     CoreNetLamda = CEF_SiAdelayCore4T60k.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.40) * 0.90
                    #     # print("If_CoreNetLamda", CoreNetLamda)
                    # else:
                    #     CoreNetLamda = CEF_SiAdelayCore4T60k.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.40)
                    #     # print("Else_CoreNetLamda", CoreNetLamda)    
                    
                    # CloudNetLamda = CEF_SiAdelayCore4T60k.get_CloudNetwork_lamda(trafic) + ((AccesLamda-AccesNetLamda) * 0.60)
                    #     # print("Else_CoreNetLamda", CoreNetLamda)  
    ...
    ## Latency that is served at local Access Network
                    if j==k and k<self.NumAcceNetwork:
                        ...
                        latency[i][j][k] = CEF_SiAdelayCore4T60k.get_latency_serve_ByAN_CEF(self.UpLinkBandWd,self.DownLinkBandWd,miuAccesNet[i][j],AccesNetLamda,lamda[i][j])
     ...
     ## Latency that is served at AN Neighbor
                        ...
                        latency[i][j][k] = CEF_SiAdelayCore4T60k.get_latency_servesd_Byneighbour_AN_CEF(self.UpLinkBandWd,self.DownLinkBandWd,miuAccesNet[i][j],AccesNetLamda,lamda[i][j],Dist_Acc_Acc) 
                        # print("latency[i][j][k]NeighbourAN",latency[i][j][k])
                        
                        # CoreNetLamda = CEF_SiAdelayCore4T60kNew.get_CoreNetwork_lamda(j,tasks)
                        # print("CoreNetLamda",CoreNetLamda)
       ...
       ## Latency that is served at local CN   3.789639064123363e-05
                        ...
                        latency[i][j][k] = CEF_SiAdelayCore4T60k.get_latency_served_ByCN_CEF(self.UpLinkBandWd,self.DownLinkBandWd,miuCoreNet[i],CoreNetLamda,lamda[i][j],Dist_Acc_Cor)#1.66666666666666666e-6
        ...
        ## Latency that is served at CN neighbour    
                        ...
                        latency[i][j][k] = CEF_SiAdelayCore4T60k.get_latency_served_Byneighbour_CN_CEF(self.UpLinkBandWd,self.DownLinkBandWd,miuCoreNet[i],CoreNetLamda,lamda[i][j],Dist_Acc_Cor,Dist_Cor_Cor)
      ...
      # Dist_Cor_Cloud=3.3333333333333333e-4
                        ...
                        latency[i][j][k]= CEF_SiAdelayCore4T60k.get_latency_served_ByClN_CEF(self.UpLinkBandWd,self.DownLinkBandWd,miuCloudNet[i],CloudNetLamda,lamda[i][j],Dist_Acc_Cor,  Dist_Cor_Cloud)#1.6666666666666666e-4
...
# system average delay 𝑙(𝑖,𝑗)
        ...
        total_traffic = sum(sum(map(sum, action)))
