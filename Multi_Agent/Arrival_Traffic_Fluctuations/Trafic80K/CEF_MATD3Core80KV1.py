...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 29 15:59:48 2022
...
# from testWithCapacity import Environment
    ...
    for _ in range(eval_episodes):
        ...
        while not done:
            ...
            for agent_id in range(len(policy)):
                ...
                obs, reward, done = env.step(actions, agent_id)
                
                # avge_latency += latency
                ...
                # rew.append(reward)
                indexx+=1
                avge_reward /= (env.NumCoreNetwork * eval_episodes)
    avge_reward /= (eval_episodes*env.NumCoreNetwork)
    # ep_reward.append(avgtot_reward)    
    # avge_latency /=(env.NumCoreNetwork*10) 
    # avgtot_latency = np.mean(evl_latency[-10:]) 
    return avge_reward
...
# Before 2000 timesteps, we play random actions
  if total_timesteps < warmup_step:
    for agent in range(len(env_agent)):
        ...
        episode_reward/10*env.NumCoreNetwork
...
# We increase the total reward
          episode_latency += latency
          # episode_latency /env.NumAcceNetwork*10
          episode_latency /10
          episode_reward += reward
          # episode_reward/env.NumAcceNetwork*10
          episode_reward/10*env.NumCoreNetwork
          action = np.array(action).flatten()
...
# ep_reward.append(ev_reward)

from datetime import datetime
now = datetime.now()
dt_string = now.strftime("%d/%m/%Y %H:%M:%S")

with open("Trainingresult.txt", "a+") as file_object:
...
def evaluate_policy(policy, eval_episodes=10):
    ...
    print ("---------------------------------------")
    print ("Average latency over the Evaluation Step: %10f" % (avge_latency))
    print ("Average Reward over the Evaluation Step: %10f" % (avge_reward))
    print ("---------------------------------------")
    from datetime import datetime
    now = datetime.now()
    dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
    
    with open("Evalutionresult.txt", "a+") as file_object:
        # Move read cursor to the start of file.
        file_object.seek(0)
        # If file is not empty then append '\n'
        data = file_object.read(100)
    ...
    return avge_reward, avge_reward 
