...
from CEF_MAEnvironmentCore2V2 import Environment
    ...
    for _ in range(eval_episodes):
        ...
        while not done:
            ...
            for agent_id in range(len(policy)):
                ...
                # rew.append(reward)
                indexx+=1
                avg_reward = tot_reward / (env.NumCoreNetwork * eval_episodes)
    avgtot_reward = avg_reward /(eval_episodes*env.NumCoreNetwork)
...
def evaluate_policy(policy, eval_episodes=10):
    ...
    # with open("Evalutionresult.csv", "w") as file_object:
    #     data= csv.writer(file_object)
    #     data.writerow('Reward')
    #     data.writerows(str(rew))
        # Move read cursor to the start of file.
        # file_object.seek(0)
        # # If file is not empty then append '\n'
        # csv.reader(file_object,100)
        # if len(data) > 0 :
        #     file_object.write("\n")
        # Append text at the end of file
        # file_object.write("\n")
        ...
        # file_object.write("\n")
        # file_object.writerow(["rewrrrr = "])
        # file_object.writer(str(rew))
