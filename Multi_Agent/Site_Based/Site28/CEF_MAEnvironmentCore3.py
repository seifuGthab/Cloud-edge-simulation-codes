    ...
    def calculate_latency(self, obs, action, agent_id):
        ...
        for i in range(len(latency)):
            ...
            for j in range(len(latency[i])):
                # AccesNetLamda = CEF_delayCore3.get_AccesNetwork_lamda(i,j, trafic)
                # # print("AccesNetLamda", AccesNetLamda)
                # if miuAccesNet<AccesNetLamda:
                #     AccesNetLamda=CEF_delayCore3.get_AccesNetwork_lamda(i,j, trafic)*0.35
                for k in range(len(latency[i][j])):
                    ...
                    CloudNetLamda = CEF_delayCore3.get_CloudNetwork_lamda(trafic) + (AccesLamda - (AccesNetLamda +((AccesLamda - AccesNetLamda) *0.3)))
                        # print("If_CoreNetLamda", CoreNetLamda) 
