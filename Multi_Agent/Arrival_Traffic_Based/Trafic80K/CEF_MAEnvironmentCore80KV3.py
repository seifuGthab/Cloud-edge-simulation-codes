...
# -*- coding: utf-8 -*-
"""
Created on Sat Jul 23 19:20:54 2022
...
# Load (Allocate) trffic    
    def Load_Traffic(self, action):
        ...
        #         # for k in range(len(task[m])):
        #         trafic.append(task[n][m])
                # trafic.sort()
        ...
        for i in range(len(latency)):
            ...
            for j in range(len(latency[i])):
                ...
                for k in range(len(latency[i][j])):
                    ...
                    CoreLamda = CEF_MAdelayCore80k.get_CoreNetwork_lamda(k,trafic) 
                    # print(" CoreLamda",  CoreLamda)
                    if (self.miuCoreNetwork[i] <= CoreLamda):
                        CoreNetLamda = (CoreLamda + ((AccesLamda-AccesNetLamda) * 0.40)) * 0.80 
                        # print("If_CoreNetLamda", CoreNetLamda)
                    else:
                        CoreNetLamda = (CEF_MAdelayCore80k.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.40)) *  0.80
                        # print("Else_CoreNetLamda", CoreNetLamda)    
                    
                    CloudLamda = CEF_MAdelayCore80k.get_CloudNetwork_lamda(trafic) + ((AccesLamda-AccesNetLamda) * 0.60)
                    if (self.miuCoreNetwork[i] <= CoreLamda):
                        CloudNetLamda = CloudLamda + (CoreLamda - CoreNetLamda)
                    else:
...
# Average Latency (Average system latency)
    ...
    def step(self, actions, agent_id):
        ...
        # for n in range(self.NumCloudNetwork):
        #     for m in range(self.NumCoreNetwork):
        #         Action.append(actions/actions.sum(axis=1)[:,None])
        #         # print("Action",Action)
