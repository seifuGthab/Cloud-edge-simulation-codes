...
# Calculate latence for given actions
    def calculate_latency(self, obs, actions, agent_id):
        latency = obs[0]
        lamda = obs[1]
        ...
        for i in range(len(latency)):
            ...
            for j in range(len(latency[i])):
                ...
                for k in range(len(latency[i][j])):
                    AccesNetLamda = CEF_MAdelayCore60k.get_AccesNetwork_lamda(i,j, action)
                    # print("AccesLamda",AccesNetLamda)
                    CoreNetLamda = CEF_MAdelayCore60k.get_CoreNetwork_lamda(j,action)
                    # print("CoreLamda",CoreLamda)
                    CloudNetLamda = CEF_MAdelayCore60k.get_CloudNetwork_lamda(action)
                    ...
                    # CoreLamda = CEF_MAdelayCore60k.get_CoreNetwork_lamda(j,trafic) 
                    ...
                    # else:
                    #     CoreNetLamda = CEF_MAdelayCore60k.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.20)
...
# Jth AN Ith CN of Cloud Latency
        ...
        for j in range(len(latency[agent_id])):
            for k in range(len(latency[agent_id][j])):
                ...
                total_latency += (actions[agent_id][j][k]*latency[agent_id][j][k])
