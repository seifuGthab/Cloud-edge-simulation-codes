...
# Arrival traffic rate at jth AN-CEF site of ith CN-CEF site
        ...
        self.lam = (8000,8000,8000,8000,8000,8000,8000,8000)
        # self.lam = (40000,40000,40000,40000,40000,40000,40000,40000)
        # self.lam = (2000,2000,2000,2000,2000,2000,2000,2000)
        # self.lam = (60000,60000,60000,60000,60000,60000,60000,60000)
        np.random.seed(1)
...
#method calling for fining distance      
    ...
    def get_observation(self):
        ...
        self.miuCloudNetwork = [3000000 for i in range(self.NumCloudNetwork)]       
        self.miuCoreNetwork = [270000 for i in range(self.NumCoreNetwork)]   
        self.miuAcceNetwork = [[10000 for i in range(self.NumAcceNetwork)] for j in range(self.NumCoreNetwork)] 
...
# Calculate latency for given actions
        ...
        for i in range(len(latency)):
            for j in range(len(latency[i])):
                ...
                for k in range(len(latency[i][j])):
                    ...
                    AccesNetLamda = CEF_delayCore480k.get_AccesNetwork_lamda(i,j, actions)
                    # print('AccesNetLamdadddddddddd', AccesNetLamda)
                    CoreNetLamda = CEF_delayCore480k.get_CoreNetwork_lamda(j, actions)
                    # print('lamdaCoreNet', CoreNetLamda)
                    CloudNetLamda = CEF_delayCore480k.get_CloudNetwork_lamda(actions)
                    # # print('lamdaAccesNet', CoreNetLamda)
                    
                    # AccesLamda = CEF_delayCore480k.get_AccesNetwork_lamda(i,j, trafic)
                    ...
                    #     # print("AccesNetLamda", AccesNetLamda)
                    # else:
                    #     AccesNetLamda = CEF_delayCore480k.get_AccesNetwork_lamda(i,j, trafic) * 0.80
                    #     # print("Else_AccesNetLamda", AccesNetLamda)
                    # CoreLamda = CEF_delayCore480k.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.40)
                    # # print("CoreNetLamda", CoreNetLamda)
                    # if (self.miuCoreNetwork[i] <= CoreLamda):
                    #     CoreNetLamda = CEF_delayCore480k.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.40) * 0.90
                    #     # print("If_CoreNetLamda", CoreNetLamda)
                    # else:
                    #     CoreNetLamda = CEF_delayCore480k.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.40)
                    #     # print("Else_CoreNetLamda", CoreNetLamda)    
                    
                    # CloudNetLamda = CEF_delayCore480k.get_CloudNetwork_lamda(trafic) + ((AccesLamda-AccesNetLamda) * 0.60)
                        # print("Else_CoreNetLamda", CoreNetLamda)  
    ...
    ## Latency that is served at local Access Network
                    if j==k and k<self.NumAcceNetwork:
                        ...
                        latency[i][j][k] = CEF_delayCore480k.get_latency_serve_ByAN_CEF(self.UpLinkBandWd,self.DownLinkBandWd,miuAccesNet[i][j],AccesNetLamda,lamda[i][j])
     ...
     ## Latency that is served at AN Neighbor
                        ...
                        latency[i][j][k] = CEF_delayCore480k.get_latency_servesd_Byneighbour_AN_CEF(self.UpLinkBandWd,self.DownLinkBandWd,miuAccesNet[i][j],AccesNetLamda,lamda[i][j],Dist_Acc_Acc) 
                        # print("latency[i][j][k]NeighbourAN",latency[i][j][k])
                        
                        # CoreNetLamda = CEF_delayCore480kNew.get_CoreNetwork_lamda(j,tasks)
                        # print("CoreNetLamda",CoreNetLamda)
       ...
       ## Latency that is served at local CN   3.789639064123363e-05
                        ...
                        latency[i][j][k] = CEF_delayCore480k.get_latency_served_ByCN_CEF(self.UpLinkBandWd,self.DownLinkBandWd,miuCoreNet[i],CoreNetLamda,lamda[i][j],Dist_Acc_Cor)#1.66666666666666666e-6
        ...
        ## Latency that is served at CN neighbour    
                        ...
                        latency[i][j][k] = CEF_delayCore480k.get_latency_served_Byneighbour_CN_CEF(self.UpLinkBandWd,self.DownLinkBandWd,miuCoreNet[i],CoreNetLamda,lamda[i][j],Dist_Acc_Cor,Dist_Cor_Cor)
      ...
      # Dist_Cor_Cloud=3.3333333333333333e-4
                        ...
                        latency[i][j][k]= CEF_delayCore480k.get_latency_served_ByClN_CEF(self.UpLinkBandWd,self.DownLinkBandWd,miuCloudNet[i],CloudNetLamda,lamda[i][j],Dist_Acc_Cor,  Dist_Cor_Cloud)#1.6666666666666666e-4
...
# latency = 1/l 
        ...
        if self.done == True:
            ...
            self.CoreNetLamda = [0 for i in range(self.NumCoreNetwork)]       
            self.CloudNetLamda = [0 for i in range(self.NumCloudNetwork)]
        # reward = (1/latency)/100000
        return reward
    
    def step(self, actions):
        
        intial_Observation, obs2, observe = self.get_observation()
        reward = self.calculate_latency(intial_Observation, actions)
...
if __name__ == "__main__":
    ...
    # print("env.get_observation()[2]",env.get_observation()[2])
