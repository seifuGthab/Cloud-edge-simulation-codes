...
# Load (Allocate) trffic    
    ...
    #     #     traf.append(task[m].sum(axis=1))
        
    #     # for i in range(len(traf)):
    #     #     for j in range(len(traf[i])):
    #     #         if traf[i][j] < self.lamda[i][j]:
    #     #             task[i][j][random.randint(0,len(task[i][j])-1)] += self.lamda[i][j] - traf[i][j]
    #     #         elif traf[i][j] > self.lamda[i][j]:
    #     #             task[i][j][random.randint(0,len(task[i][j])-1)] -= traf[i][j] - self.lamda[i][j]
    #     #         else:
    #     #             continue
    #     return task
    def Load_Traffic(self, actions):
...
# Calculate latency for given actions
        ...
        for i in range(len(latency)):
            for j in range(len(latency[i])):
                ...
                for k in range(len(latency[i][j])):
                    ...
                    AccesNetLamda = CEF_delayCore3.get_AccesNetwork_lamda(i,k, trafc)
                    # print('AccesNetLamdadddddddddd', AccesNetLamda)
                    CoreNetLamda = CEF_delayCore3.get_CoreNetwork_lamda(k, trafc)
                    ...
                    # CoreLamda = CEF_delayCore3.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.40)
                    # # print("CoreNetLamda", CoreNetLamda)
                    # if (self.miuCoreNetwork[i] <= CoreLamda):
                    #     CoreNetLamda = CEF_delayCore3.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.40) * 0.90
                    #     # print("If_CoreNetLamda", CoreNetLamda)
                    # else:
                    #     CoreNetLamda = CEF_delayCore3.get_CoreNetwork_lamda(j,trafic) + ((AccesLamda-AccesNetLamda) * 0.40)
    ...
    ## Latency that is served at local Access Network
     ...
     ## Latency that is served at AN Neighbor
                        ...
                        # CoreNetLamda = CEF_delayCore3New.get_CoreNetwork_lamda(j,tasks)
                        # print("CoreNetLamda",CoreNetLamda)
